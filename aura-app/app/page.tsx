...
export default function Home() {
  ...
  // 1. КРИТИЧЕСКИЙ ШАГ: Сообщаем Base App, что приложение загружено.
  // Без этого вызова нативные действия (createCast) будут игнорироваться.
  useEffect(() => {
    const init = async () => {
  ...
  const handleShare = async (e: React.MouseEvent) => {
    ...
    try {
      // 2. ОТКРЫВАЕМ РЕДАКТОР ПОСТА (CREATE CAST)
      ...
      // Фолбек только для обычного браузера
      navigator.clipboard.writeText(`${shareText}\n\n${appUrl}`);
      alert("Results copied! Open Base App or Warpcast to share.");
    }
  };
  ...
  return (
    <main className="app-container" onMouseDown={handlePulseTrigger} onTouchStart={handlePulseTrigger}>
      ...
      <style jsx global>{`
        ...
        .header, .ritual-main, .mini-wallet-btn, .ritual-btn, .share-btn { pointer-events: auto; }
