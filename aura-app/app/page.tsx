...
export default function Home() {
  ...
  const friends = [
    { addr: '0x838aD0EAE54F99F1926dA7C3b6bFbF617389B4D9', name: '0x838a...B4D9' },
    { addr: '0x02feeb0AdE57b6adEEdE5A4EEea6Cf8c21BeB6B1', name: 'zizzamia.eth' }
  ...
  return (
    <main className={`app-container ${stage}`}>
            ...
            {friends.map(f => (
              <div key={f.addr} className="friend-card" onClick={() => triggerImpulse(f.addr)}>
                <div className="avatar-wrapper">
                  <Avatar address={f.addr as `0x${string}`} className="friend-avatar" />
      ...
      <style jsx global>{`
        ...
        .vibrant-wallet { background: #fff !important; color: #000 !important; border-radius: 100px !important; padding: 10px 24px !important; border: none !important; transition: 0.3s !important; }
        .vibrant-wallet:hover { transform: scale(1.05); box-shadow: 0 0 20px rgba(255,255,255,0.4) !important; }
        ...
        .core.splash { animation: splashAnim 0.8s ease-out; }

        @keyframes splashAnim {
          0% { transform: scale(1.4); box-shadow: 0 0 60px var(--glow); }
          50% { transform: scale(2.5); box-shadow: 0 0 160px var(--glow); filter: brightness(1.8); }
        ...
        .description { font-size: 0.9rem; color: #aaa; margin-bottom: 25px; font-style: italic; line-height: 1.6; }
        ...
        .friend-card { display: flex; align-items: center; background: rgba(0,0,0,0.3); padding: 15px 20px; border-radius: 25px; cursor: pointer; transition: 0.3s; border: 1px solid rgba(255,255,255,0.05); }
        .friend-card:hover { background: rgba(255,255,255,0.08); border-color: rgba(255,255,255,0.2); }
        
        /* МЕРЦАЮЩАЯ АУРА ИКОНКИ */
        .avatar-wrapper { position: relative; width: 44px; height: 44px; display: flex; align-items: center; justify-content: center; }
        .friend-avatar { width: 40px !important; height: 40px !important; border-radius: 50%; z-index: 2; border: 1px solid rgba(255,255,255,0.2); }
        .orb-glow { position: absolute; inset: -2px; border-radius: 50%; z-index: 1; animation: multiColorGlow 3s infinite alternate; filter: blur(4px); opacity: 0.7; }

        @keyframes multiColorGlow {
          0% { box-shadow: 0 0 10px #a855f7; transform: scale(1); }
          33% { box-shadow: 0 0 15px #06b6d4; transform: scale(1.1); }
          66% { box-shadow: 0 0 10px #10b981; transform: scale(0.95); }
          100% { box-shadow: 0 0 15px #f59e0b; transform: scale(1.05); }
        }
